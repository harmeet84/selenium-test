...
public class TestChromeBrowser {
	...
	public Map<Integer, Map<String, String>> retrievalMap = new HashMap();
	...
	public void testChelseaYoaApplication() {
		...
		for (int i= 1 ; i <= 1 ; i++ ) {
		driver.navigate().to(dataMap().get(i).get("URL").toString());
		...
		driver.findElement(By.name("clientId")).sendKeys(dataMap().get(i).get("clientId").toString());
		driver.findElement(By.name("clientSecretId")).sendKeys(dataMap().get(i).get("clientSecretId").toString());
		
		WebElement mySelectElement = driver.findElement(By.id("bankId"));
		Select dropdown= new Select(mySelectElement);
		//driver.findElement(By.id("bankId")).sendKeys("value", "YBS");
		dropdown.selectByVisibleText(dataMap().get(i).get("bankId").toString());
		...
		testbPsd2Login1Page(i);
		testcPsd2Login2Page(i);
		...
		testdPsd2TppRetrurnPage(i);
		testdPsd2TppResultPage(i);
	...
	//@Test (priority=2)
	public void testbPsd2Login1Page(int i) {
...
//		driver.findElement(By.name("uid")).sendKeys(dataMap.get("uid").toString());
		element.sendKeys(dataMap().get(i).get("uid").toString());		
		...
		System.out.println("Page title b: - "+strPageTitle);
		if ("Chelsea Building Society - User Login".equalsIgnoreCase(strPageTitle)) {
	...
	//@Test (priority=3)
	public void testcPsd2Login2Page(int i) {
		...
		dropdownDay.selectByVisibleText(dataMap().get(i).get("dateOfBirth.day").toString());
		dropdownMonth.selectByVisibleText(dataMap().get(i).get("dateOfBirth.month").toString());
		dropdownYear.selectByVisibleText(dataMap().get(i).get("dateOfBirth.year").toString());
	...
	public void testdPsd2AccountSelectionPage() {
		...
		System.out.println("Page title: - "+strPageTitle);
		if ("Chelsea Building Society - Authorisation accounts".equalsIgnoreCase(strPageTitle)) {
	...
	public void testdPsd2AuthorisationRequestPage() {
		...
		System.out.println("Page title: - "+strPageTitle);
		if ("Chelsea Building Society - Authorisation request".equalsIgnoreCase(strPageTitle)) {
	...
	public void testdPsd2AuthorisationTransactionServicePage() {
		...
		System.out.println("Page title: - "+strPageTitle);
		if ("Chelsea Building Society - Transaction Confirmation service".equalsIgnoreCase(strPageTitle)) {
	...
	public void testdPsd2AuthorisationTransactionPasswordPage() {
		...
		System.out.println("Page title: - "+strPageTitle);
		if ("Chelsea Building Society -".equalsIgnoreCase(strPageTitle)) {
	...
	//@Test (priority=9)
	public void testdPsd2TppRetrurnPage(int i) {
		...
		System.out.println("Page title: - "+strPageTitle);
		driver.findElement(By.name("clientId")).sendKeys(dataMap().get(i).get("clientId").toString());
		driver.findElement(By.name("clientSecret")).sendKeys(dataMap().get(i).get("clientSecretId").toString());
	...
	//@Test (priority=10)
	public void testdPsd2TppResultPage(int i) {
	...
	public void testdPsd2TppResultPage() {
		...
		for (int i=0 ; i < radiobutton.size() ; i++) {
			...
			if ("GetAccounts".equalsIgnoreCase(radiobutton.get(i).getAttribute("value")) && counter == 0) {
				...
				boolean flag = wholeBodyText.getText().contains("Error Details:");
				if (flag) {
					logger.log(LogStatus.FAIL, "Test Fail - GetAccountBalances");
				}
